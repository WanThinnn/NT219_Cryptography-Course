        ...
        static void ProcessInput(string action, string[] args)
        {
            if (action == "genkey")
            {
                if (args.Length != 3)
                {
                    ...
                    Console.Error.WriteLine("\t {0} genkey <KeyFileFormat> <KeyFile>", Environment.GetCommandLineArgs()[0]);
                    return;
                }
                GenerateAndSaveIV_Keys(args[1], args[2]);
            }
