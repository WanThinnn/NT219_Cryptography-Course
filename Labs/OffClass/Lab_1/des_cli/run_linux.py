...
def run_encrypt_test_cases(i):
    ...
    with open(f"./result_linux/case_{i}/case_{i}_result_encrypt.txt", "w") as f:
        ...
        for command in commands:
            ...
            cipher_file_path = f"./result_linux/case_{i}/case_{i}_cp_{mode}.txt"
            if "encrypt" in command:
                command = f"./DES__CLI {command} key_DES.key DER {case_file_path} Text {cipher_file_path} DER"
...
def run_decrypt_test_cases(i):
    ...
    with open(f"./result_linux/case_{i}/case_{i}_result_decrypt.txt", "w") as f:
        ...
        for command in commands:
            ...
            cipher_file_path = f"./result_linux/case_{i}/case_{i}_cp_{mode}.txt"
            # Tạo tên tệp dựa trên 'mode'
            rcv_file_path = f"./result_linux/case_{i}/case_{i}_rcv_{mode}.txt"
            if "decrypt" in command:
                command = f"./DES__CLI {command} key_DES.key DER {rcv_file_path} Text {cipher_file_path} DER"
...
# Nhớ tạo key trước khi gọi 2 hàm bên dưới: ./DES__CLI genkey DER key_DES.key",
i = 6
