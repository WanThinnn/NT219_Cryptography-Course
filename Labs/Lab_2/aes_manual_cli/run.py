...
def run_encrypt_test_cases(i):
    ...
    with open(f"./result/case_{i}/case_{i}_result_encrypt.txt", "w") as f:
        ...
        for command in commands:
            ...
            cipher_file_path = f"./result/case_{i}/case_{i}_cp.txt"
            if "encrypt" in command:
                command = f"main.exe {command} HEX key_AES.key Text {case_file_path} HEX {cipher_file_path}"
...
def run_decrypt_test_cases(i):
    ...
    with open(f"./result/case_{i}/case_{i}_result_decrypt.txt", "w") as f:
        ...
        for command in commands:
            cipher_file_path = f"./result/case_{i}/case_{i}_cp.txt"
            # Tạo tên tệp dựa trên 'mode'
            rcv_file_path = f"./result/case_{i}/case_{i}_rcv.txt"
            if "decrypt" in command:
                command = f"main.exe {command} HEX key_AES.key HEX {cipher_file_path} Text {rcv_file_path}"
