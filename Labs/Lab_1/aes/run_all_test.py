...
def encryption_test(i):
    with open(f"./result/all_results_case_{i}.txt", "w") as f:
        ...
        remove_extra_blank_lines(1, "encrypt")
        f.write("\n**********************************************************\n")
        f.write(f"Result for Encypt 8 mode in test case {i} with 100 time running: \n")
        f.write(f"Execution time: {execution_time} seconds\n")
        f.write("**********************************************************\n")


def decryption_test(i):
    with open(f"./result/all_results_case_{i}.txt", "w") as f:
        # run_genkey_test_cases()
        start_time = time.time()  # Lưu thời gian bắt đầu
        # for _ in range(number_of_runs):
        ...
        end_time = time.time()  # Lưu thời gian kết thúc
        execution_time = end_time - start_time  # Tính thời gian thực thi
        remove_extra_blank_lines(1, "decrypt")
        f.write("\n**********************************************************\n")
        f.write(f"Result for Decrypt 8 mode in test case {i} with 100 time running: \n")
        f.write(f"Execution time: {execution_time} seconds\n")
        f.write("**********************************************************\n")
