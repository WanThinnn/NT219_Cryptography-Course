...
def get_enc_file_path(case_i):
    return f"./result/case_{case_i}/case_{case_i}_result_encrypt.txt"

def get_dec_file_path(case_i):
    return f"./result/case_{case_i}/case_{case_i}_result_decrypt.txt"



def show_result(i):
    file_path = f"./result/case_{i}/sum_time_case_{i}.txt"    
    with open(file_path, "w") as f:
        enc_file = get_enc_file_path(i)
        mode_times = calculate_enc_mode_times(enc_file)
        f.write(f"- File: {enc_file}\n")
        f.write("- Encryption Time Per Modes: \n")
        for mode, time in mode_times.items():
            f.write(f"   + Mode: {mode} - Total time: {time/1000} seconds ~ {time/(1000*10)} miliseconds\n")
        ...
        f.write("- Decryption Time Per Modes: \n")
        for mode, time in mode_times.items():
            f.write(f"   + Mode: {mode} - Total time: {time/1000} seconds ~ {time/(1000*10)} miliseconds\n")    
